...
def get_args_parser():
    ...
    parser.add_argument(
        '--disable_scaleup', action='store_true',
    ...
    parser.add_argument(
        '--augmentor_ver', default='v1', type=str, choices=['v1', 'v2'],
    ...
    parser.add_argument(
        '--temporal_module_name', default=None, type=str, metavar='TEM',
    ...
    parser.add_argument('--no_token_mask', action='store_true',
                        default=False, help='do not apply token mask')
    parser.add_argument(
        '--temporal_heads_scale', default=1.0, type=float,
        help='scale of the number of spatial heads')
    parser.add_argument('--temporal_mlp_scale', default=1.0,
                        type=float, help='scale of spatial mlp')
    parser.add_argument('--rel_pos', action='store_true', default=False,
                        help='use relative positioning in temporal module]')
    parser.add_argument(
        '--temporal_pooling', type=str, default=None,
    ...
    parser.add_argument('--super_img_rows', default=1, type=int,
...
#    parser.add_argument('--input-size', default=224, type=int, help='images input size')
    ...
    parser.add_argument(
        '--opt-betas', default=None, type=float, nargs='+', metavar='BETA',
    ...
    parser.add_argument(
        '--lr-noise', type=float, nargs='+', default=None, metavar='pct, pct',
    ...
    parser.add_argument(
        '--cooldown-epochs', type=int, default=10, metavar='N',
    ...
    parser.add_argument(
        '--train-interpolation', type=str, default='bicubic',
    ...
    parser.add_argument(
        '--cutmix-minmax', type=float, nargs='+', default=None,
        help='cutmix min/max ratio, overrides alpha and enables cutmix if set (default: None)')
    parser.add_argument(
        '--mixup-prob', type=float, default=1.0,
        help='Probability of performing mixup or cutmix when either/both is enabled')
    parser.add_argument(
        '--mixup-switch-prob', type=float, default=0.5,
        help='Probability of switching to cutmix when both mixup and cutmix enabled')
    parser.add_argument(
        '--mixup-mode', type=str, default='batch',
...
#                        type=str, help='semantic granularity')
    ...
    parser.add_argument('--no-resume-loss-scaler',
                        action='store_false', dest='resume_loss_scaler')
    parser.add_argument('--no-amp', action='store_false', dest='amp', help='disable amp')
    parser.add_argument(
        '--use_checkpoint', default=False, action='store_true',
    ...
    # exp
    parser.add_argument('--simclr_w', type=float, default=0.,
                        help='weights for simclr loss')
    parser.add_argument('--contrastive_nomixup', action='store_true',
                        help='do not involve mixup in contrastive learning')
    parser.add_argument('--temperature', type=float,
                        default=0.07, help='temperature of NCE')
    parser.add_argument('--branch_div_w', type=float, default=0.,
                        help='add branch divergence in the loss')
    parser.add_argument('--simsiam_w', type=float, default=0.,
                        help='weights for simsiam loss')
    ...
    parser.add_argument('--dml_w', type=float, default=0.,
                        help='enable deep mutual learning')
    ...
    parser.add_argument('--selfdis_w', type=float, default=0.,
...
def main(args):
    ...
    num_classes, train_list_name, val_list_name, test_list_name, \
        filename_seperator, image_tmpl, filter_video, label_file = get_dataset_config(
...
#    std = IMAGENET_DEFAULT_STD
    ...
    simclr_criterion = simclr.NTXent(
        temperature=args.temperature) if args.simclr_w > 0. else None
    ...
    mean = (
        0.5, 0.5, 0.5) if 'mean' not in model.default_cfg else model.default_cfg['mean']
    std = (
        0.5, 0.5, 0.5) if 'std' not in model.default_cfg else model.default_cfg['std']
    ...
    dataset_train = video_data_cls(
        args.data_dir, train_list, args.duration, args.frames_per_group,
        num_clips=args.num_clips, modality=args.modality, image_tmpl=image_tmpl,
        dense_sampling=args.dense_sampling, transform=train_augmentor, is_train=True,
        test_mode=False, seperator=filename_seperator, filter_video=filter_video)

    num_tasks = utils.get_world_size()
    data_loader_train = build_dataflow(dataset_train, is_train=True,
                                       batch_size=args.batch_size,
                                       workers=args.num_workers,
                                       is_distributed=args.distributed)

    val_list =val_list_name
    ...
    dataset_val = video_data_cls(
        args.data_dir, val_list, args.duration, args.frames_per_group,
        num_clips=args.num_clips, modality=args.modality, image_tmpl=image_tmpl,
        dense_sampling=args.dense_sampling, transform=val_augmentor, is_train=False,
        test_mode=False, seperator=filename_seperator, filter_video=filter_video)

    data_loader_val = build_dataflow(
        dataset_val, is_train=False, batch_size=args.batch_size, workers=args.num_workers,
        is_distributed=args.distributed)

    if args.eval:
        test_stats = evaluate(
            data_loader_val, model, device, num_tasks, distributed=True, amp=args.amp,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        test_stats = evaluate(data_loader_val, model, device,
                              num_tasks, distributed=True, amp=args.amp)
        print(
            f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc1']:.1f}%")
